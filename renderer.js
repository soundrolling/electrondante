...
async function requestMicrophonePermission() {
  try {
    ...
    showMessage('Requesting microphone permission - please look for a system dialog. If no dialog appears, check System Settings → Privacy & Security → Microphone', 'info');
    ...
    if (result.granted) {
      ...
      const errorMsg = result.message || 'Microphone permission denied';
      showMessage(errorMsg, 'error');
      addLog(`Permission denied: ${errorMsg}`, 'error');
      
      // Provide helpful instructions
      if (errorMsg.includes('System Settings')) {
        addLog('To fix: Open System Settings → Privacy & Security → Microphone → Enable this app', 'info');
      }
    }
  } catch (error) {
    const errorMessage = error.message || 'Unknown error';
    showMessage(`Error requesting permission: ${errorMessage}`, 'error');
