    ...
    try {
      ...
      const { data, fromCache } = await cachedFetch(
        ...
        () => _fetchProjectsFromNetwork(uid, email),
        {
          ...
          force,
          onUpdate: (fresh) => {
            // Background refresh arrived — update UI silently
      ...
      if (fromCache) {
        // Served from IndexedDB immediately; background refresh already fired
        _applyProjects(data, uid);
      } else {
        // Was a blocking network fetch (cache miss or force)
        _applyProjects(data ?? [], uid);
      }
    } catch (e) {
